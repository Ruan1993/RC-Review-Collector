(function() {
  ...
  fetch(API_URL)
    .then(response => {
      if (!response.ok) throw new Error(`Failed to load reviews: ${response.status} ${response.statusText}`);
  ...
  function renderError(msg) {
    shadow.innerHTML = `
      ...
      <div class="widget-container">
        <div class="error">Unable to load reviews<br><small>${msg || ''}</small></div>
      </div>
    `;
  }
  ...
  function renderWidget(data) {
    ...
    if (!data.reviews || data.reviews.length === 0) {
      console.warn('No reviews found in data');
      renderError('No reviews found');
      return;
    }
    ...
    const starsHtml = Array(5).fill(0).map((_, i) => 
      `<span style="opacity: ${i < Math.round(data.rating || 5) ? 1 : 0.3}">${starSvg}</span>`
    ).join('');
    ...
    shadow.innerHTML = `
      ...
      <div class="widget-container">
        <div class="header">
          <div class="rating-badge">${data.rating || 5} ★</div>
          <div>
            <div class="stars" style="display:flex; gap:2px; margin:0;">${starsHtml}</div>
            <div class="total-reviews">${data.user_ratings_total || data.reviews.length} Google Reviews</div>
          </div>
        </div>
